/**
 * Red neuronal Codrops (mismo efecto que bg-neural-canvas) solo en el panel blanco del navbar al hover.
 */
...
if (panel && canvas && canHover && !prefersReducedMotionGlobal) {
  ...
  function buildPoints() {
    points = [];
    const stepX = width / GRID_DIV;
    const stepY = height / GRID_DIV;
    ...
    for (let i = 0; i < points.length; i += 1) {
      ...
      p.closest = [];
      for (let k = 0; k < LINK_COUNT && k < scored.length; k += 1) {
  ...
  panel.addEventListener("pointerenter", (event) => {
    ...
    animating = true;
    panel.classList.add("nav__brand-panel--hover");
    startLoop();
  });
  ...
  panel.addEventListener("pointerleave", () => {
    animating = false;
    panel.classList.remove("nav__brand-panel--hover");
    startLoop();
  });
